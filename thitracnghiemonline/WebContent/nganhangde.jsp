<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">    
    <title>Thi Trac Nghiem Online</title>    
    <link href="css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" type="text/css" href="canhchinh.css">
    <style> 
      body{
        padding-top: 20px;
        padding-bottom: 70px;
        padding-left: 150px;
        padding-right: 150px;      
      }
       </style>
  </head>
  <body>    
    
    <div class="contaner">
       <div id="anh" class="col-md-12">
        <div  class="col-md-2">
          <img src="LOGO CLC.jpg" align="left" width="80" hight="50"class="img-responsive"  class="img-rounded"> 
        </div>       
        <div  class="col-md-8">
          <h1 align="center">Thi Trac Nghiem Online</h1>
        </div> 
        <!-- anh admin -->
        <!-- <div id="anh" class="col-md-12"> -->
        <div  class="col-md-2">
          <img src="anhnguoiquantrikithi.png" align="right" width="160" hight="120"class="img-responsive"  class="img-rounded"> 
        <!-- </div> -->
        <!--  -->
       </div>     
      </div>
      <p></p>
      <div class="row">
          <div class="col-md-12" >          
            <ul class="nav nav-tabs backgroup">
              <li class="col-md-2"><a href=""> <font color="yellow">Trang Chu</font> </a> </li>
              <li class="col-md-2"><a href=""> <font color="white">Huong Dan</font> </a></li>
              <li class="col-md-2"><a href=""><font color="white">Tin Tuc</font></a></li>
              <li class="col-md-2"><a href=""><font color="white">Lien He</font></a></li>
            </ul>
          </div>          
       </div>
      </div>
      
        
    </div>

    <!-- kfnksgnks -->
    <div class="container-fluid">
      <div class="row">
      <!-- quan ly tai khoan -->       
       <div class="col-md-2 sidebar backgroup2 col-center">
          <ul class="nav nav-sidebar ">
            <li><a href="#"></a></li>
            <li><a href="#"></a></li>
            <li><a href="#"></a></li>
            <li><a href="#"></a></li>
            <li class="active backgroup3"><a href="#"><font color="yellow">WEB</font> <span class="sr-only">(current)</span></a></li> 
            <li><a href="#">CONG NGHE PHAN MEM</a></li>
            <li><a href="#">DBMS</a></li>
            <li><a href="#">OOP</a></li>
            <li><a href="#">C++</a></li>
            <li><a href="#">C#</a></li>
            <li><a href="#">JAVA</a></li>
            <li><a href="file:D:\namhoc_2016_2017\hk1_2016_2017\laptrinhweb\thuc ti\bootstrap-3.3.7-dist/Trangchu.html">Thoat</a></li>
            <li><a href="#"></a></li>
            <li><a href="#"></a></li>
            <li><a href="#"></a></li>
            <li><a href="#"></a></li>
            <li><a href="#"></a></li>
            <li><a href="#"></a></li>
            <li><a href="#"></a></li>
            <li><a href="#"></a></li>
            <li><a href="#"></a></li>
            <li><a href="#"></a></li>
            <li><a href="#"></a></li>
            <li><a href="#"></a></li>
          </ul>          
        </div>

        <div class="col-sm-10  main">
          <div class="table-responsive">
            <table class="table table-striped">
             <thead>
                <tr>

                  <th><a href="file:D:\namhoc_2016_2017\hk1_2016_2017\laptrinhweb\thuc ti\bootstrap-3.3.7-dist/themtungcau.html">Them</a></th>
                  <th><a href="#">Sua</a></th>
                  <th><a href="#">Xoa</a></th>
                  <th><a href="file:D:\namhoc_2016_2017\hk1_2016_2017\laptrinhweb\thuc ti\bootstrap-3.3.7-dist/themfile.html">Import File/Excel</a></th>
                </tr>
              </thead>
            </table>
            <table class="table table-striped">
               <thead>
                <tr>
                  <th>STT</th>
                  <th>Noi dung cau hoi</th>
                  <th>Nguoi tao</th>
                  <th>Cau tra loi</th>
                </tr>
              </thead>
              <tbody>
               <tr>
                  <td><label><input type="radio" name="optionsRadios" id="optionsRadios2" value="option2">
                      </label>1</td>
                  <td>Cau hoi 1</td>
                  <td>Man</td>                   
                  <td><img src="cautraloi.png" align="left" width="80" hight="50"class="img-responsive"  class="img-rounded"> </td>
                </tr> 
                <tr>
                  <td><label><input type="radio" name="optionsRadios" id="optionsRadios2" value="option2">
                      </label>2</td>
                  <td>Cau hoi 2</td>
                  <td>Man</td>                   
                  <td><img src="cautraloi.png" align="left" width="80" hight="50"class="img-responsive"  class="img-rounded"> </td>
                </tr> 
                <tr>
                  <td><label><input type="radio" name="optionsRadios" id="optionsRadios2" value="option2">
                      </label>3</td>
                  <td>Cau hoi 3</td>
                  <td>Man</td>                   
                  <td><img src="cautraloi.png" align="left" width="80" hight="50"class="img-responsive"  class="img-rounded"> </td> 
                </tr> 
                <tr>
                  <td><label><input type="radio" name="optionsRadios" id="optionsRadios2" value="option2">
                      </label>4</td>
                  <td>Cau hoi 4</td>
                  <td>Man</td>                   
                  <td><img src="cautraloi.png" align="left" width="80" hight="50"class="img-responsive"  class="img-rounded"> </td>
                </tr> 
                <tr>
                  <td><label><input type="radio" name="optionsRadios" id="optionsRadios2" value="option2">
                      </label>5</td>
                  <td>Cau hoi 5</td>
                  <td>Man</td>                   
                  <td><img src="cautraloi.png" align="left" width="80" hight="50"class="img-responsive"  class="img-rounded"> </td>
                </tr> 
                <tr>
                  <td><label><input type="radio" name="optionsRadios" id="optionsRadios2" value="option2">
                      </label>6</td>
                  <td>Cau hoi 6</td>
                  <td>Man</td>                   
                  <td><img src="cautraloi.png" align="left" width="80" hight="50"class="img-responsive"  class="img-rounded"> </td>
                </tr> 
                <tr>
                  <td><label><input type="radio" name="optionsRadios" id="optionsRadios2" value="option2">
                      </label>7</td>
                  <td>Cau hoi 7</td>
                  <td>Man</td>                   
                  <td><img src="cautraloi.png" align="left" width="80" hight="50"class="img-responsive"  class="img-rounded"> </td>
                </tr> 
                <tr>
                  <td><label><input type="radio" name="optionsRadios" id="optionsRadios2" value="option2">
                      </label>8</td>
                  <td>Cau hoi 8</td>
                  <td>Man</td>                   
                  <td><img src="cautraloi.png" align="left" width="80" hight="50"class="img-responsive"  class="img-rounded"> </td>
                </tr> 
                <tr>
                  <td><label><input type="radio" name="optionsRadios" id="optionsRadios2" value="option2">
                      </label>9</td>
                  <td>Cau hoi 9</td>
                  <td>Man</td>                   
                  <td><img src="cautraloi.png" align="left" width="80" hight="50"class="img-responsive"  class="img-rounded"> </td>
                </tr> 
                <tr>
                  <td><label><input type="radio" name="optionsRadios" id="optionsRadios2" value="option2">
                      </label>10</td>
                  <td>Cau hoi 10</td>
                  <td>Man</td>                   
                  <td><img src="cautraloi.png" align="left" width="80" hight="50"class="img-responsive"  class="img-rounded"> </td>
                </tr> 
                
            
              </tbody>
            </table>
          </div>
              
    </div>           
                        

    <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.12.4/jquery.min.js"></script>    
    <script src="js/bootstrap.min.js"></script>
  </body>
</html>


